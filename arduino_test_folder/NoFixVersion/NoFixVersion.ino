...
void loop() {
 ...
 // if(GPS.fix==1) { //Only save data if we have a fix
  ...
  itoa (((analogRead(x)-xinit)),  an0, 10);    // X read and convert numeric analog pin to char
  itoa (((analogRead(y)-yinit)),  an1, 10);    // Y  ..
    
  //switch for float 
  itoa ((((analogRead(z)-zinit)),  an2, 10);    // Z converted to char (the reacion for the +9.8 is to compensate for gravity)
