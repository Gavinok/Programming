...
void loop() {
  ...
  itoa (((analogRead(x)-xinit)/68.5*9.8),  an0, 10);    // X read and convert numeric analog pin to char
  itoa (((analogRead(y)-yinit)/68.5*9.8),  an1, 10);    // Y  ..
    
  //switch for float 
  itoa ((((analogRead(z)-zinit)/68.5*9.8)+9.8),  an2, 10);    // Z converted to char (the reacion for the +9.8 is to compensate for gravity)
